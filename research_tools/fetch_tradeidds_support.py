...
def main():
 ...
 # F: official origin distribution of re-exports to Russia; API does not permit origin x commodity simultaneously.
 ...
 # H: value and quantity for precious-metal/diamond imports and key machinery/electronics re-exports.
 six=set(); byrev={}
 ...
 for module,ttype,partner,codes in modules:
  for sv in ('VCm','QCm'):
   for group in chunks(codes,20):
    ...
    if st.get('name') not in ('Success',) and 'No record found' not in msg: raise RuntimeError(f'H query failed {st}')
    rows=j.get('dataSet',[])
    for x in rows: x['_module']=module
    combined.extend(rows); logs.append({'module':module,'stat':sv,'codes':group,'params':p,'requested_url':req,'final_url':final,'status':st,'records':len(rows),'file':fn,'sha256':sha(b)})
 ...
 hmeta={'extract_id':'H_key_hs_quantity','source':'Hong Kong C&SD Trade-IDDS','period':'2012-01/2026-06','modules':{'precious_import':list(import_prefixes),'key_reexport':list(export_prefixes)},'statistics':['VCm','QCm'],'records':len(hdf),'api_calls':len(logs),'stable_hs6_intersection_count':len(stable6),'combined_sha256':sha(hb),'retrieved_at_utc':datetime.now(timezone.utc).isoformat()}
