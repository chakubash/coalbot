...
def get_retry(url,*,params=None,timeout=240,attempts=15):
...
# I: official HKMA period-average HKD per USD, retrieved in yearly blocks to avoid gateway limits.
...
for year in range(2012,2027):
 ...
 params={'from':f'{year}-01','to':f'{year}-{end_month:02d}','pagesize':100,'offset':0}
 r=get_retry(url,params=params); j=r.json(); pages.append({'params':params,'payload':j})
 if not j.get('header',{}).get('success'): raise RuntimeError(j.get('header'))
 records.extend(j.get('result',{}).get('records',[]))
...
expected=pd.period_range('2012-01','2026-06',freq='M').astype(str); missing=sorted(set(expected)-set(fx.period))
if missing: raise RuntimeError(f'HKMA calendar incomplete: {missing[:12]}')
...
refs={}
for rev in ('H4','H5','H6'):
 u=f'https://comtradeapi.un.org/files/v1/app/reference/{rev}.json'; r=get_retry(u); p=OUT/f'L_official_{rev}.json'; p.write_bytes(r.content); refs[rev]=r.json()['results']
rows=[]; all_ids=sorted(set().union(*[{str(x['id']) for x in refs[r] if str(x.get('id','')).isdigit() and int(x.get('aggrlevel',-1)) in (2,4,6)} for r in refs])); lookup={r:{str(x['id']):x for x in refs[r]} for r in refs}
for code in all_ids:
 levels=[int(lookup[r][code]['aggrlevel']) for r in refs if code in lookup[r]]; level=levels[0]; row={'hs_code':code,'hs_level':level}
 ...
 row['stable_all_three']=all(row[f'in_{x}'] for x in ('HS2012','HS2017','HS2022')); row['audit_action']='retain_native_and_stable' if row['stable_all_three'] else ('aggregate_to_hs2_or_exclude_from_longitudinal_hs6' if level in (4,6) else 'retain_hs2'); rows.append(row)
...
page='https://www.worldbank.org/en/research/commodity-markets'; candidates=['https://thedocs.worldbank.org/en/doc/5d903e848db1d1b83e0ec8f744e55570-0350012021/related/CMO-Historical-Data-Monthly.xlsx','https://thedocs.worldbank.org/en/doc/5d903e848db1d1b83e0ec8f744e55570-0350012021/related/CMO-Historical-Data-Monthly.xlsx?download=1']
try:
 pr=get_retry(page); (OUT/'world_bank_commodity_page.html').write_bytes(pr.content); soup=BeautifulSoup(pr.text,'html.parser')
...
wbp=OUT/'M_world_bank_cmo_monthly.xlsx'; wbp.write_bytes(wb); xl=pd.ExcelFile(wbp); sheet=next((x for x in xl.sheet_names if 'Monthly' in x and 'Price' in x),xl.sheet_names[0]); rawdf=pd.read_excel(wbp,sheet_name=sheet,header=None)
header_idx=None
for i in range(min(30,len(rawdf))):
...
cols={'Gold':'gold_usd_per_troy_oz','Silver':'silver_usd_per_troy_oz','Platinum':'platinum_usd_per_troy_oz'}; out=pd.DataFrame({'period':pd0['period']}); found={}
