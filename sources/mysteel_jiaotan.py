...
BASE_URL = "https://jiaotan.mysteel.com"

GOOD_HINTS = [
    "焦煤", "炼焦煤", "焦炭", "煤焦", "配焦煤", "主焦煤", "瘦煤", "肥煤", "喷吹煤"
]
...
def collect_links(max_pages: int = 10):
    ...
    for url in URLS[:max_pages]:
        try:
            html = fetch_html(url)
            if not html:
                print(f"DEBUG_MYSTEEL_JIAOTAN_EMPTY {url}")
                continue
            ...
            print(f"DEBUG_MYSTEEL_JIAOTAN_PAGE {url}: {page_hits}")

        except Exception as e:
            print(f"DEBUG_MYSTEEL_JIAOTAN_ERROR {url}: {e}")

    print(f"DEBUG_MYSTEEL_JIAOTAN_TOTAL: {len(results)}")
    return results
