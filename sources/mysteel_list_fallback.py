...
def collect_links(max_pages: int = 3):
    results = []
    seen = set()
    ...
    for url in URLS[:max_pages]:
        try:
            html = fetch_html(url)
            if not html:
                print(f"DEBUG_MYSTEEL_LIST_EMPTY {url}")
                continue
            ...
            print(f"DEBUG_MYSTEEL_LIST_PAGE {url}: {page_hits}")

        except Exception as e:
            print(f"DEBUG_MYSTEEL_LIST_ERROR {url}: {e}")

    print(f"DEBUG_MYSTEEL_LIST_TOTAL: {len(results)}")
    return results
