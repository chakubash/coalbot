...
⬛ Новости из источников
▪️ Mysteel
1. Перспектива простоев шахт в Шаньси вновь попала в утренний обзор Mysteel. Новых раскрытых деталей по причинам, масштабам и срокам пока нет. 7月8日陕北动力煤价格以降为主.
Ссылка: javascript:void(0)'''

BAD_CLIENT_RU = '''⬛ Оперативная сводка по рынку угля КНР за 8 июля 2026 года

Период: с 20:00 7 июля до 09:27 8 июля по Пекину

⬛ Картина дня
Рынок следит за ценами и безопасностью добычи.

⬛ Новости из источников
За период не было свежих значимых сообщений от Mysteel, SXCoal и CLS.

⬛ Иные новости
— Baosteel — запустила новый производственный проект.
Ссылка: https://news.google.com/rss/articles/CBMi-bad

— Baosteel — заказала новую трубную линию.
Ссылка: https://news.google.com/rss/articles/CBMi-bad2'''

BAD_EMPTY_SOURCE_RU = '''⬛ Оперативная сводка по рынку угля КНР за 8 июля 2026 года
...
def _assert_good_ru(ru):
    ...
    assert "news.google.com/rss/articles" not in ru, ru
    assert "перспектива простоев" not in ru.lower(), ru
    assert "новых раскрытых деталей" not in ru.lower(), ru
    assert "материал не раскрывает" not in ru.lower(), ru
    assert "в доступной части" not in ru.lower(), ru


def main():
    cleaned_payload = reports._client_news_for_prompt([
        {"client": "Baosteel", "title": "Baosteel caster", "url": "https://example.com/baosteel", "snippet": "caster production"},
        {"client": "Baosteel", "title": "Baosteel second caster", "url": "https://example.com/baosteel-2", "snippet": "caster production"},
        {"client": "Ansteel", "title": "Ansteel furnace", "url": "https://news.google.com/rss/articles/CBMi-bad", "snippet": "blast furnace"},
        {"client": "Valin", "title": "Valin rolling mill", "resolved_url": "https://example.com/valin", "url": "https://news.google.com/rss/articles/CBMi-old", "snippet": "rolling mill"},
    ])
    assert [item["client"] for item in cleaned_payload] == ["Baosteel", "Valin"], cleaned_payload
    assert all("news.google.com/rss/articles" not in item.get("url", "") for item in cleaned_payload), cleaned_payload
    ...
    retried_zh, markers, prompts = _run_build([_response(GOOD_RU, BAD_ZH), _response()])
    assert markers == ["daily_source_llm"], markers
    assert len(prompts) == 2, prompts
    assert "оператив" not in retried_zh["zh"], retried_zh["zh"]
    _assert_good_ru(retried_zh["ru"])
    ...
    no_client_items, markers, prompts = _run_build([_response(BAD_CLIENT_RU, GOOD_ZH), _response(BAD_CLIENT_RU, GOOD_ZH)])
    assert markers == ["daily_source_llm"], markers
    assert len(prompts) == 2, prompts
    assert "news.google.com/rss/articles" not in no_client_items["ru"], no_client_items["ru"]
    assert no_client_items["ru"].count("— Baosteel") == 0, no_client_items["ru"]
    assert "Свежих релевантных новостей по отслеживаемым компаниям за период не найдено." in no_client_items["ru"], no_client_items["ru"]

    empty_source, markers, prompts = _run_build([_response(BAD_EMPTY_SOURCE_RU, GOOD_ZH), _response()])
    assert markers == ["daily_source_llm"], markers
    assert len(prompts) == 2, prompts
    _assert_good_ru(empty_source["ru"])
