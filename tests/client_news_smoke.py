...
def main():
    ...
    try:
        ...
        cleaned = clean_accepted_client_items([
            {"client": "Baosteel", "title": f"Baosteel caster project {i}", "url": f"https://example.com/baosteel-clean-{i}", "snippet": "Baosteel caster production line commissioned."}
            for i in range(3)
        ] + [
            {"client": "Ansteel", "title": "Ansteel blast furnace maintenance", "url": "https://example.com/ansteel-clean", "snippet": "Ansteel blast furnace maintenance."},
            {"client": "Valin", "title": "Valin rolling mill starts", "url": "https://example.com/valin-clean", "snippet": "Valin rolling mill production starts."},
            {"client": "Yongfeng", "title": "Yongfeng coke oven maintenance", "url": "https://example.com/yongfeng-clean", "snippet": "Yongfeng coke oven maintenance."},
            {"client": "Risun", "title": "Risun coke contract", "url": "https://example.com/risun-clean", "snippet": "Risun coke procurement contract."},
            {"client": "Ben Steel", "title": "Ben Steel sinter plant resumes", "url": "https://example.com/bensteel-clean", "snippet": "Ben Steel sinter plant resumes."},
        ], max_items=5)
        assert len(cleaned) == 5, cleaned
        assert sum(1 for item in cleaned if item["client"] == "Baosteel") == 1, cleaned
        assert all("news.google.com/rss/articles" not in item.get("url", "") for item in cleaned), cleaned
        assert all("original_google_url" not in item for item in cleaned), cleaned
    finally:
